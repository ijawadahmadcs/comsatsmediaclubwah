...
export default function AdminsPage() {
  ...
  async function create(event: FormEvent) {
    ...
    setForm({ registrationNumber: "", password: "", name: "" });
  ...
  async function update(
    ...
    const result = await response.json();
    setMessage(response.ok ? success : result.message);
    if (response.ok) void load();
  }
  async function remove(admin: Admin) {
    if (!window.confirm(`Delete ${admin.registrationNumber}?`)) return;
    const response = await fetch(`/api/admins/${admin._id}`, {
      method: "DELETE",
    });
    const result = await response.json();
    setMessage(response.ok ? "Administrator deleted." : result.message);
    if (response.ok) void load();
  }
  return (
    <main className="min-h-screen bg-[#08090b] px-5 py-16 text-white sm:px-10 lg:px-16">
      <div className="mx-auto max-w-6xl">
        ...
        <h1 className="mt-4 text-4xl font-semibold">Admin Management</h1>
        ...
        <form
          onSubmit={create}
          className="mt-8 grid gap-4 rounded-2xl border border-white/10 bg-[#0b0d10] p-6 sm:grid-cols-4"
        >
          <input
            required
          ...
          <table className="w-full min-w-[720px] text-left text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                admins.map((admin) => (
                  <tr key={admin._id} className="border-b border-white/[0.06]">
                    <td className="p-4 text-white/80">{admin.name || "-"}</td>
                    <td className="p-4 text-white/60">
                      {admin.registrationNumber}
                    </td>
                    <td className="p-4 text-white/60">{admin.role}</td>
                    <td className="p-4 text-white/60">
